...
*Setup instructions for a gitian build of Learnium using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Learnium
...
to the Learnium GitHub release page.
...
- [Building Learnium](#building-learnium-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/learnium-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/learnium-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/learnium-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/learnium-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/learnium-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/learnium-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/learnium-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@learnium.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/learnium-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/learnium-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/learnium-osx-*-build.assert
```
