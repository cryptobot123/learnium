Learnium version 3.0.2 is now available from:
...
Learnium v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zLRA spends, automint calculation adjustments, and other various updates/fixes.

zLRA spending requires this update.
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the Learnium address as it's first parameter and the passphrase as it's second parameter.
...
- #292 `feadab4` Additional checks for double spending of zLra serials. (presstab)
...
- #291 `cb314e6` [Qt] zLra control quantity/amount fixes (rejectedpromise)
