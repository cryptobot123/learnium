...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    bool fZerocoinActive = nHeight - 2 >= Params().Zerocoin_StartHeight();
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zlras that have been in mempool long
                    //and higher priority to zlras that are large in value
                    ...
                    // zLRA spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zLRA spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zLRA serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (pblock->nVersion >= 4 && pblock->nVersion < 7) {
            if (fZerocoinActive) {
                uint256 nCheckpoint;
                ...
                if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                    //For the period before v2 activation, zLRA will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zLRA with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }

                //Sign block with the zLRA key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zLRA key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	if (std::strstr(pblock->ToString().c_str(), "0000044697856d823ab7b6db9c83d1513fd8eca4b72287072b5934f353eeced6")) {
		LogPrintf("Genesis Block\n");
        } else if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash()) {
            return error("LearniumMiner : generated block is stale");
    	}
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("LearniumMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("LearniumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("learnium-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running LearniumMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("LearniumMiner exception");
    } catch (...) {
        LogPrintf("LearniumMiner exception");
    }

    LogPrintf("LearniumMiner exiting\n");
}
