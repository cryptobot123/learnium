...
#include "qt/learnium/send.h"
#include "qt/learnium/forms/ui_send.h"
#include "qt/learnium/addnewcontactdialog.h"
#include "qt/learnium/qtutils.h"
#include "qt/learnium/sendchangeaddressdialog.h"
#include "qt/learnium/optionbutton.h"
#include "qt/learnium/sendconfirmdialog.h"
#include "qt/learnium/myaddressrow.h"
#include "qt/learnium/guitransactionsutils.h"
...
SendWidget::SendWidget(LearniumGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (LRA)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a Learnium address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 LRA");
	...
	setCssProperty(coinIcon, "coin-icon-lra");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_LRA, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any LRA to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress lraAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(lraAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(lraAdd.Get(), dialog->getLabel().toStdString(),
				AddressBook::AddressBookPurpose::SEND)) {
