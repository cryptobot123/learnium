...
#include "qt/learnium/topbar.h"
#include "qt/learnium/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/learnium/lockunlock.h"
#include "qt/learnium/qtutils.h"
#include "qt/learnium/receivedialog.h"
...
TopBar::TopBar(LearniumGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopLra}, "amount-small-topbar");
    setCssProperty(ui->labelAmountLra, "amount-topbar");
    setCssProperty({ui->labelPendingLra}, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("Learnium")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // LRA Total
    CAmount lraAvailableBalance = balance;

    // Set
    QString totalLra = GUIUtil::formatBalance(lraAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopLra->setText(totalLra);

    // Expanded
    ui->labelAmountLra->setText(totalLra);

    ui->labelPendingLra->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureLra->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
