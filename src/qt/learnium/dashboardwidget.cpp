...
#include "qt/learnium/dashboardwidget.h"
#include "qt/learnium/forms/ui_dashboardwidget.h"
#include "qt/learnium/sendconfirmdialog.h"
#include "qt/learnium/txrow.h"
...
DashboardWidget::DashboardWidget(LearniumGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of LRA staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareLra, "square-chart-lra");
    setCssProperty(ui->labelLra, "text-chart-lra");
    ...
    ui->labelAmountLra->setText("0 LRA");
    setCssProperty(ui->labelAmountLra, "text-stake-lra-disable");
    ...
    ui->btnHowTo->setText(tr("How to get LRA"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("LRA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            lra = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalLra += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesLra.append(lra);

        int max = std::max(lra, lra);
...
void DashboardWidget::onChartRefreshed() {
    ...
    set0->append(chartData->valuesLra);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalLra > 0) {
        setCssProperty(ui->labelAmountLra, "text-stake-lra");
    } else {
        setCssProperty(ui->labelAmountLra, "text-stake-lra-disable");
    }
    forceUpdateStyle({ui->labelAmountLra});
    ui->labelAmountLra->setText(GUIUtil::formatBalance(chartData->totalLra, nDisplayUnit));
