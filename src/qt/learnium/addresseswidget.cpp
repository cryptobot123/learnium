...
#include "qt/learnium/forms/ui_addresseswidget.h"
#include "qt/learnium/addresslabelrow.h"
#include "qt/learnium/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(LearniumGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a Learnium address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress lraAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(lraAdd.Get(), label.toUtf8().constData(),
                lraAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
