...
#include "zlracontroldialog.h"
#include "ui_zlracontroldialog.h"
...
std::set<std::string> ZLraControlDialog::setSelectedMints;
std::set<CMintMeta> ZLraControlDialog::setMints;

bool CZLraControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZLraControlDialog::COLUMN_DENOMINATION || column == ZLraControlDialog::COLUMN_VERSION || column == ZLraControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZLraControlDialog::ZLraControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZLraControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zLRA Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZLra->setProperty("cssClass", "text-main-purple");
    ui->labelZLra_int->setProperty("cssClass", "text-main-purple");
...
ZLraControlDialog::~ZLraControlDialog()
{
    delete ui;
}

void ZLraControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLraControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZLraControlWidgetItem* itemDenom(new CZLraControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZLraControlWidgetItem *itemMint = new CZLraControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zLRA.";
            else if (!mint.isSeedCorrect)
                strReason = "The zLRA seed used to mint this zLRA is not the same as currently hold in the wallet";
            else
...
void ZLraControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLraControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZLra_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZLraControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZLraControlDialog::GetSelectedMints()
{
...
void ZLraControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
