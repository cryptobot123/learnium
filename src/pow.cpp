...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock)
{
    ...
    /* current difficulty formula, learnium - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, unsigned int nHeight)
{
    bool fNegative;
    bool fOverflow;
    uint256 bnTarget;
    ...
    if (hash > bnTarget) {
        if (Params().MineBlocksOnDemand())
            ...
            return error("CheckProofOfWork() : hash doesn't match nBits, %s ... %s .. %s", hash.ToString().c_str(), bnTarget.ToString().c_str(), hashTarget.ToString().c_str());
    }
