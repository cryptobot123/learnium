...
CZLraStake::CZLraStake(const libzerocoin::CoinSpend& spend)
{
...
int CZLraStake::GetChecksumHeightFromMint()
{
...
int CZLraStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZLraStake::GetChecksum()
{
    return nChecksum;
}

// The zLRA block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZLraStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZLraStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zLRA staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZLraStake::GetUniqueness()
{
    //The unique identifier for a zLRA is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZLraStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZLraStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zLRA that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZLRAOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLRA output", __func__);
    vout.emplace_back(outReward);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZLRAOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLRA output", __func__);
...
//!LRA Stake
bool CLraStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CLraStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CLraStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CLraStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CLraStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CLraStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CLraStake::GetUniqueness()
{
    //The unique identifier for a LRA stake is the outpoint
...
CBlockIndex* CLraStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
