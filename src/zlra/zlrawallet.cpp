...
CzLRAWallet::CzLRAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlra seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLRASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLRASeed_deprecated()) {
                    LogPrintf("%s: Updated zLRA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlra seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlra wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLRAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLRACount(nCountLastUsed);
    else if (!walletdb.ReadZLRACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLRAWallet::Lock()
{
    seedMaster = 0;
}

void CzLRAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLRAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLRA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLRAWallet::LoadMintPoolFromDB()
{
...
void CzLRAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLRAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLRAWallet::SyncWithChain(bool fGenerateMintPool)
{
...
void CzLRAWallet::SeedToZLRA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLRAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLRAWallet::GenerateDeterministicZLRA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLRAWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLRAWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLRAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
