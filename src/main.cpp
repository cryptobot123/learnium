...
#include "zlra/accumulators.h"
#include "zlra/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "Learnium cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulra) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
		return error("%s : zLRA spend with serial %s is already in block %d\n", __func__,
...
{
	//Check to see if the zLRA is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		try {
			if (!spend->HasValidSignature())
				return error("%s: V2 zLRA spend does not have a valid signature\n", __func__);
		}
		...
		if (spend->getSpendType() != expectedType) {
			return error("%s: trying to spend zLRA without the correct spend type. txid=%s\n", __func__,
				tx.GetHash().GetHex());
		}
	}
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state)
{
	...
	for (const CTxIn& txin : tx.vin) {
		...
		if (isPublicSpend) {
			...
			if (!ZLRAModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
		...
		if (isPublicSpend) {
			...
			if (!ZLRAModule::validateInput(txin, prevOut, tx, ret)) {
...

	{
		...
		if (hasZcSpendInputs) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("%s : zLRA spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!isPrivZerocoinSpend && !isPublicSpend) {
					return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
						__func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zlra");
				}
				...
				if (isPublicSpend) {
					libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
					PublicCoinSpend publicSpend(params);
					if (!ZLRAModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
						return false;
					}
					...
					if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
						return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
							__func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zlra");
					}
			...
			// Check that zLRA mints (if included) are not already known
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
	...
	if (block.IsProofOfWork()) {
		if (!CheckProofOfWork(block.GetHash(), block.nBits, chainActive.Height()))
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 115000 && nHeight > 85000) {
		nSubsidy = 10 * COIN;
	}
	...
	else if (nHeight <= 195000 && nHeight > 175000) {
		nSubsidy = 15 * COIN;
	}
	else if (nHeight <= 225000 && nHeight > 195000) {
		nSubsidy = 25 * COIN;
	}
	else if (nHeight <= 245000 && nHeight > 225000) {
		nSubsidy = 15 * COIN;
	}
	else if (nHeight <= 260000 && nHeight > 245000) {
		nSubsidy = 10 * COIN;
	}
	else if (nHeight <= 275000 && nHeight > 260000) {
		nSubsidy = 7 * COIN;
	}
	else if (nHeight <= 305000 && nHeight > 275000) {
		nSubsidy = 5 * COIN;
	}
	else if (nHeight <= 365000 && nHeight > 305000) {
		nSubsidy = 3 * COIN;
	}
	else if (nHeight <= 465000 && nHeight > 365000) {
		nSubsidy = 2 * COIN;
	}
	else if (nHeight <= 665000 && nHeight > 465000) {
		nSubsidy = 1 * COIN;
	}
	else if (nHeight <= 965000 && nHeight > 665000) {
		nSubsidy = 0.5 * COIN;
	}
	else {
		nSubsidy = 0.1 * COIN;
    ...
    if (nHeight > 1000) {
        if (nHeight % 1440 == 0) {
            ...
            LogPrintf("Special Reward 50x GetBlockValue %i\n", nHeight);
            return nSubsidy;
        }
        if (nHeight % 864 == 0) {
            nSubsidy = nSubsidy * 10;
            LogPrintf("Special Reward 10x GetBlockValue %i\n", nHeight);
            return nSubsidy;
        }
        if (nHeight % 144 == 0) {
            nSubsidy = nSubsidy * 2;
            LogPrintf("Special Reward 2x GetBlockValue %i\n", nHeight);
            return nSubsidy;
        }
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZLRAStake)
{
	int64_t ret = 0;
	...
	if (nHeight > 1015) {
		if (nHeight % 1440 == 0) {
			LogPrintf("Special Reward 50x GetMasternodePayment %i\n", nHeight);
			ret = ret/50;
			return ret;
		}
        if (nHeight % 864 == 0) {
            LogPrintf("Special Reward 10x GetMasternodePayment %i\n", nHeight);
            ret = ret/10;
			return ret;
        }
            if (nHeight % 144 == 0) {
		...
		for (const CTxIn& in : tx.vin) {
			...
			if (in.IsZerocoinSpend() || isPublicSpend) {
				...
				if (isPublicSpend) {
					...
					if (!ZLRAModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		 ...
		 * note we only undo zerocoin databasing in the following statement, value to and from Learnium
...
{
	RenameThread("learnium-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	for (unsigned int i = 0; i < block.vtx.size(); i++) {
		...
		if (tx.HasZerocoinSpendInputs()) {
			...
			// Check that zLRA mints are not already known
			if (tx.HasZerocoinMintOutputs()) {
				for (auto& out : tx.vout) {
					if (!out.IsZerocoinMint())
						continue;
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
	...
	if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits, chainActive.Height()))
		return state.DoS(50, error("CheckBlockHeader() : proof of work failed"),
			REJECT_INVALID, "high-hash");

	// Version 4 header must be used after Params().sZerocoin_StartHeight(). And never before.
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
	...
	if (outs >= 3 && lastOut.scriptPubKey != tx.vout[outs - 2].scriptPubKey) {
		...
		// cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 LRA.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zLRA spends in this block
		if (tx.HasZerocoinSpendInputs()) {
			for (const CTxIn& txIn : tx.vin) {
				bool isPublicSpend = txIn.IsZerocoinPublicSpend();
				if (txIn.IsZerocoinSpend() || isPublicSpend) {
					...
					if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zLRA serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
	...
	if (block.nBits != nBitsRequired) {
		// Learnium Specific reference to the block with the wrong threshold was used.
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> lraInputs;
		std::vector<CTxIn> zLRAInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			if (stakeIn.IsZerocoinSpend()) {
				zLRAInputs.push_back(stakeIn);
			}
			else {
				lraInputs.push_back(stakeIn);
			}
		}
		const bool hasLRAInputs = !lraInputs.empty();
		const bool hasZLRAInputs = !zLRAInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasLRAInputs) {
					...
					for (const CTxIn& lraIn : lraInputs) {
						if (lraIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zLRA inputs.
			if (hasZLRAInputs) {
				for (const CTxIn& zLraInput : zLRAInputs) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zLraInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zlra");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// Learnium: We use certain sporks during IBD, so check to see if they are
		...
		if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
			try {
				...
				if (!GetAccumulatorValue(height, den, bnAccValue)) {
					LogPrint("zlra", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
					Misbehaving(pfrom->GetId(), 50);
				}
				else {
			...
			try {
				...
				if (gen.isValid(chainActive.Height())) {
					if (!lightWorker.addWitWork(gen)) {
						LogPrint("zlra", "%s : add genwit request failed \n", __func__);
						CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
