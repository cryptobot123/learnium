...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZLRAStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (sporkManager.IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZLRAStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZLRAStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    if (pindexPrev->nHeight < 3885) {
    	masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZLRAStake);
    } else {
	masternodePayment = GetMasternodePayment(pindexPrev->nHeight + 1, blockValue, 0, fZLRAStake);
    }
