...
public:
    ...
    bool WriteZLRASeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZLRASeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZLRASeed_deprecated(uint256& seed);
    bool EraseZLRASeed();
    bool EraseZLRASeed_deprecated();

    bool WriteZLRACount(const uint32_t& nCount);
    bool ReadZLRACount(uint32_t& nCount);
